...
function NavGroup({ item }: { item: NavItem }) {
  ...
  return (
    <div>
      <button
        className="flex w-full items-center gap-2 px-3 py-2 text-sm rounded-md hover:bg-muted text-muted-foreground hover:text-foreground transition-colors"
        onClick={() => setOpen((v) => !v)}
      >
        {item.icon}
      ...
      {open && (
        <div className="ml-6 mt-0.5 space-y-0.5 border-l border-border pl-3">
          {item.children.map((child) => (
            <button
              key={child.label}
              className="flex w-full items-center px-2 py-1.5 text-sm rounded-md hover:bg-muted text-muted-foreground hover:text-foreground transition-colors"
            >
              {child.label}
            </button>
          ))}
    ...
    <div className="rounded-lg border border-border overflow-hidden flex" style={{ height: 380 }}>
      ...
      <div className="flex-1 flex flex-col">
        ...
        <div className="flex-1 p-4 text-sm text-muted-foreground">
          Main content area. Click items in the sidebar to see collapsible groups.
        </div>
      </div>
    </div>
  )
